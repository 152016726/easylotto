...
//					this.data = this.datatmp.responseXML.cloneNode(true);
				...
				if (true || this.Method != this.METHOD1) {
					...
					//alert("check state error "+e.message);
					}
				}
...
//		var ARR_ACTIVEX = ["Microsoft.XmlDom", "Microsoft.XMLHTTP"];
		...
		try {
			...
			if (window.XMLHttpRequest) {
				...
				alert("windows xmlhttprequest error "+e.message);
				}
			//} else if (typeof window.ActiveXObject == "undefined") {
				...
				for (i=0; tmp && i<ARR_ACTIVEX2.length; i++) {
					...
					//alert("activex2 error "+e.message);
					}
				}
...
*/
				if (this.Method != this.METHOD3) {
					this.datatmp.open("GET", this.link + '?nocache=' + (''+(new Date())).replace(/ /, '_'), this.Async);
if(navigator.userAgent.match(/MSIE 10/))this.datatmp.responseType = 'msxml-document';
//alert("userAgent is"+navigator.userAgent);
if(navigator.userAgent.match(/rv:11.0/)){
this.datatmp.responseType = 'msxml-document';
}
					this.datatmp.send(null);
				} else {
					this.datatmp.load(this.link + '?nocache=' + (''+(new Date())).replace(/ /, '_'));
				}
				i = 50;
...
//			window.status = this.StatusString[this.status] + ' : ' + i + ' - ' + e;
			this.status = this.Fail;
			//alert("download error is"+e.message);
		}
	}	// download
...
function xmlGetNode(node, xpath) {
	...
	try {
		...
		//alert("call xmlgetNode IE9 or IE11 88");
		//var req;
		//req=new ActiveXObject("Msxml2.XMLHTTP");
		//node = req.responseXML;
		// node.responseType("msxml-document");
			tmp = node.selectSingleNode(xpath);
		//} else if(typeof window.ActiveXObject == "undefined"){
		//alert("call xmlgetNode IE11 99");
		//tmp = node.selectSingleNode(xpath);
		//}else{
		}catch(e){
		try{
		//alert("call xmlgetNode not IE 88");
			tmp2 = node.evaluate(xpath, node, null, XPathResult.ANY_TYPE, null);
			tmp = tmp2.iterateNext();
			//tmp = node.selectSingleNode(xpath);
			} catch (e) {
		tmp = null;
		//alert("xmlGetNode error is "+e.message);
	}
		}
